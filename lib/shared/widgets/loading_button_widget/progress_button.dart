...
enum ButtonState { normal, loading, success, fail }
...
class ProgressButton extends StatefulWidget {
  ...
  ProgressButton(
      ...
      this.minWidthStates = const <ButtonState>[ButtonState.loading, ButtonState.fail, ButtonState.success],
  ...
  factory ProgressButton.icon({
    ...
    List<ButtonState> minWidthStates = const <ButtonState>[ButtonState.loading, ButtonState.fail, ButtonState.success],
  }) {
    assert(
    ...
    Map<ButtonState, Widget> stateWidgets = {
      ButtonState.normal: buildChildWithIcon(iconedButtons[ButtonState.normal]!, iconPadding, textStyle),
      ButtonState.loading: const Column(),
      ButtonState.fail: const Column(),
      ButtonState.success: const Column()
    };

    Map<ButtonState, Color> stateColors = {
      ButtonState.normal: iconedButtons[ButtonState.normal]!.color,
...
class _ProgressButtonState extends State<ProgressButton> with TickerProviderStateMixin {
  ...
  Color? get backgroundColor =>
      colorAnimation == null ? widget.stateColors[widget.state!] : colorAnimation!.value ?? widget.stateColors[widget.state!];
  ...
  void initState() {
    ...
    progressIndicator = widget.progressIndicator ??
        CircularProgressIndicator(backgroundColor: widget.stateColors[widget.state!], valueColor: const AlwaysStoppedAnimation<Color>(Colors.white));
  }
  ...
  Widget getButtonChild(bool visibility) {
    ...
    } else if (widget.state == ButtonState.fail) {
      return Row(
        mainAxisAlignment: widget.progressIndicatorAlignment,
        children: <Widget>[
          SizedBox(
    ...
    return AnimatedOpacity(opacity: visibility ? 1.0 : 0.0, duration: const Duration(milliseconds: 250), child: buttonChild);
  }
        ...
        return AnimatedContainer(
            ...
            child: MaterialButton(
              ...
              shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(widget.radius), side: const BorderSide(color: Colors.transparent, width: 0)),
