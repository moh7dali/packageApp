...
class BranchRepositoryImpl implements BranchRepository {
  ...
  @override
  Future<Either<AppFailure, BranchDetailsList>> getAllBranches({Map<String, dynamic>? body}) async {
   
      try {
        final branchDetailsList = await remoteDataSource.getBrandBranches(body: body!);
        return Right(branchDetailsList);
      } on ApiErrorsException catch (e) {
        return Left(AppFailure(
            errorsModel: ErrorsModel(errorCode: e.errorCode, errorMessage: e.errorMessage ?? 'somethingWrong'.tr),
            failureType: FailureType.serverFailure));
  ...
  @override
  Future<Either<AppFailure, BranchDetails?>> getClosestBranch({required Map<String, dynamic> body}) async {
   
      try {
        final branchDetails = await remoteDataSource.getClosestBranch(body: body);
        return Right(branchDetails);
      } on ApiErrorsException catch (e) {
        return Left(AppFailure(
            errorsModel: ErrorsModel(errorCode: e.errorCode, errorMessage: e.errorMessage ?? 'somethingWrong'.tr),
